...
const Signup = () => {
	...
	const regUser = async (e) => {
		e.preventDefault();
		setLoading(false);
		const res = await fetch("https://photocorner33.onrender.com/user/new", {
			method: "POST",
				...
				<Main>
					<Form onSubmit={regUser} className='shadow-lg text-sm bg-slate-200'>
						...
						<div className='min-w-[200px] w-4/5 flex flex-col justify-center'>
							...
							<input
								className='w-full border border-white bg-white/30 focus:border-blue-700 focus:ring-1 focus:ring-sky-500 mt-1 h-[40px] px-2 rounded-md outline-none text-black
                       bg-transparen '
								type='text'
                       ...
                       bg-transparen '
						...
						<div className='min-w-[200px] mt3 w-4/5 flex mt-4 flex-col justify-center'>
							...
							<input
								className='w-full border border-white bg-white/30 focus:border-blue-700 focus:ring-1 focus:ring-sky-500 mt-1 h-[40px] px-2 rounded-md outline-none text-black
                 bg-transparen '
								placeholder='Choose a passoward'
						...
						<div className='flex w-full mb-1 text-black items-center justify-center'>
							...
							<Link to='/login' className='text-blue-600 ml-6'>
								{" "}
								Log in
							</Link>
						</div>
